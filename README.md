...
- ✅ **判定时机可配置**（`sustain_judge_timing`，默认 `both`）：LLM 处理期间与回复后两个时机都判定，也可只选其一或重叠时一轮只判一次
...
> - `both`（默认）：LLM 处理期间（含工具循环）与回复后两个时机都判定，消息不丢且接话自然
> - `either`：任一时机判定，窗口重叠时一轮只判一次（per_round 下严格保持“一轮只判一次”）
